...
unsigned long button_event_time = 0;
...
void check_push_button(unsigned long time_now) {
  ...
  */
  ...
  if (1 == last_button_state) {  // button was not pressed
    button_event_time = BUTTON_PRESSED ? time_now : 0;
  }
  if ((button_event_time != 0) && BUTTON_PRESSED) {
    // the button is held down and we're counting
    if (time_now > button_event_time + LONG_PRESS) {
      erase_flag = 1;
      #ifdef DEBUG_LOGGING
      Serial.println("long_press");
      #endif
    }
    else if (time_now > button_event_time + SHORT_PRESS) {
      logging_enabled = 1;
      #ifdef DEBUG_LOGGING
...
void dump_data_to_serial(void){
  ...
  */
  // read the data out if necessary
  if(Serial.available() > 4) {
    ...
    if(str.substring(0) == "dump") {
      ...
      sprintf(debug_string, "0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x", result0, result1, result2, result3, result4, result5);
      Serial.println(debug_string);
